...
def publish_windows(x64_only: bool):
    ...
    bootstrap_path = f"SS14.Launcher.Bootstrap/bin/Release/{TFM}-windows/win-x64/publish/Orbitra Launcher.exe"
    # Natively compiled copy we need to get from a separate worker.
    if os.path.isfile("Orbitra Launcher.exe"):
        bootstrap_path = "Orbitra Launcher.exe"
    shutil.copyfile(bootstrap_path, "bin/publish/Windows/Orbitra Launcher.exe")
